...
**남은 것은 Phase 4(`/me/compose` 본체)와 동적 OG 이미지뿐입니다.**

Phase 3과 6을 Phase 1에 흡수했습니다. 폐기하기로 한 GOAT를 TSX로 옮겼다가
다시 지우는 것, 그리고 대체하기로 한 휘발성 꽃을 옮겼다가 다시 쓰는 것은
둘 다 버릴 코드를 쓰는 일이라 이관 시점에 함께 처리하는 편이 낫습니다.

**단, DB에 닿는 화면은 아직 실물로 검증되지 않았습니다.** 새 Supabase
프로젝트가 없어 로그인 이후 경로(온보딩·내 묘비·헌화·공개 묘비)를 돌려볼 수
없습니다. 빌드·타입·린트와 비인증 라우트 동작까지만 확인된 상태입니다.
