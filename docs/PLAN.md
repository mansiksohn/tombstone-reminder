...
| 언어 | **TypeScript** |
| GOAT 섹션 | **폐기** |
| 온보딩 챗 | **폐지.** 이름·생일·데스마스크는 묘비를 세운 뒤 꾸미는 선택 사항 |
| 메인 시나리오 | **2단계** — 랜딩(안내+프롬프트 복사) → 만들기(붙여넣기→각인→게시) |
| 로그인 | **게시 직전에만.** 그전 초안은 브라우저(sessionStorage)에 |
...
app/
  ...
  new/page.tsx                    ★ 붙여넣기 → 한 줄 각인 → 미리보기 → 게시
  auth/callback/route.ts          Supabase PKCE 코드 교환
  me/page.tsx                     내 묘비 — 이름·생일·데스마스크 꾸미기, 공유 링크
...
| 온보딩 진행도 | `is_onboarded` (첫 저장에 true, 신뢰 불가) | **없음** — 온보딩 자체를 폐지 |
...
## 3. 온보딩 폐지 — 메인 시나리오 2단계로

온보딩 챗은 축소가 아니라 **폐지**했다. 이름·생일·데스마스크를 묻던 3단계는
묘비를 만들기도 전에 서 있던 문턱이었고, 확산을 막는 요소였다.
...
|---|---|---|---|
| 1 | `/` 랜딩 | 안내 + 프롬프트 복사 | 불필요 |
| 2 | `/new` 만들기 | 붙여넣기 → 각인할 문장 선택 → 미리보기 → 게시 | **게시 직전에만** |

이름·생일·데스마스크는 묘비가 선 뒤 `/me`에서 **선택적으로 꾸미는 요소**로
내려갔다. 없어도 묘비는 선다 (이름이 비면 '신원미상').

### 로그인을 게시 직전으로 미룬 대가

게시 전까지의 작업은 DB가 아니라 브라우저에 있다. `sessionStorage`에 초안을
두고(`lib/draft.ts`), 게시 버튼을 누르면 초안을 맡긴 뒤 구글로 보냈다가,
돌아왔을 때 꺼내어 이어서 게시한다. 초안이 있다는 것은 이미 게시를 눌렀다는
뜻이므로 돌아온 뒤 다시 묻지 않는다.

탭을 닫으면 사라지는 `sessionStorage`를 쓴 것은 의도적이다. 남의 기기에
남에게 보일 추도문이 남아 있을 이유가 없다.
...
| **4** | ~~★ `/new`~~ ✅ — 붙여넣기 → 문장 선택 각인 → 미리보기 → 게시 | 만들기 흐름 완성 |
| **8** | ~~메인 시나리오 2단계 개편~~ ✅ — 온보딩 폐지, 로그인을 게시 직전으로, 꾸미기는 `/me`로 | 문턱 8단계 → 2단계 |
...
**Phase 5·7의 남은 항목까지 모두 끝났습니다.** 동적 OG 이미지, keep-alive
워크플로, 미사용 에셋 정리 전부 반영됐습니다.

## 7-1. 실기기 검증 (2026-09-10)

`npm run e2e`는 스텁 위에서 도는 검사라 확인하지 못하는 것들이 있다
(`e2e/README.md` 참조). 그 목록을 실제 기기와 실제 계정으로 밟은 결과다.

| 항목 | 결과 |
|---|---|
| 모바일 구글 로그인 | 성공 |
| 연타 제한 (1분 10송이) | 동작 — "잠시 후에 다시 놓아주세요" 노출 |
| 카톡 카드 렌더링 | 정상. 일부 특수문자가 깨지나 대응 범위 밖으로 판단 |
| 모바일 키보드 | 문제 없음 |
| 계정 삭제 | 동작. 계정과 묘비가 함께 사라짐 |
| 헌화 버튼 위치 겹침 | 지장 없음. 미세 조정은 추후 |

여기서 하나가 나왔다 — **모바일에서 헌화 버튼의 '+'가 보이지 않았다.**
`@media (hover: none)` 아래에서 글자색을 배경과 같은 grey-333으로 두고
있었다. 헌화는 방문자가 할 수 있는 유일한 행동인데 그 버튼이 빈 그릇으로만
보였다. 고쳤고, 누르는 동안 그릇이 초록으로 차오르는 :active를 붙였다.

남은 것은 **실제 LLM 답변 3종(ChatGPT·Claude·Gemini)을 붙여넣어 보는 것**과
**RLS 확인(초안이 남에게 보이지 않는지)** 이다.
