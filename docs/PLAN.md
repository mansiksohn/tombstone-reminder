...
## 7-2. 질문 확정과 흐름 마무리 (2026-09-11)

**실제 LLM 답변 3종 확인 완료.** ChatGPT·Claude·Gemini 모두 ① 거절 없이
답하고 ② 첫 문장이 각인으로 쓸 만한 형태로 오고 ③ 실명·소속이 새지 않는다.

여기 오기까지 한 번 돌아갔다. 질문을 기호로 감싼 주문 형태로 써봤는데
제미나이와 GPT는 받아들였지만 **클로드가 주입 시도로 보고 거절**했다.
대문자 토큰과 연산자형 제약이 시스템 지시문을 사칭하는 모양이라 내용이
무해해도 껍데기만으로 걸린다. 부적 같은 느낌은 UI가 맡기로 하고 질문은
평문으로 확정했다.

**게시 후 완료 화면을 없앴다.** "묘비가 세워졌습니다"라고 해놓고 링크
한 줄만 보여줘서, 세워졌다는 묘비를 정작 못 본 채 흐름이 끝났다. 이제
게시가 끝나면 곧장 `/me`로 간다. 공유 링크·복사·묘비 보기는 거기에도
있으므로 잃는 것이 없다.

**RLS 확인 완료.** 실제 DB에 anon 키로 직접 물어 게시된 것만 읽히는 것을
확인했다. 앱을 건너뛰는 것이 요점이다 — 페이지도 OG 이미지도 헌화 API도
쿼리에 `status='published'`를 직접 붙이므로, RLS가 통째로 꺼져 있어도
화면에서는 아무 이상이 없다. 반면 anon 키는 브라우저 번들에 구워져
나가므로 누구나 꺼내서 직접 부를 수 있다.

처음 물었을 때 `[]`가 왔는데 그것만으로는 통과가 아니었다. 빈 테이블을
조회해도 같은 `[]`가 나오기 때문이다. 묘비를 게시해 그 한 줄이 나오는
것을 보고, 다시 비공개로 돌려 사라지는 것까지 본 뒤에야 증명이 됐다.

이 절차를 **`npm run check-rls`** 로 넣어뒀다. 게시된 행이 하나도 없으면
통과라고 말하지 않고 판단 보류로 끝난다 — 위의 함정을 그대로 막는다.
스키마를 다시 적용하거나 프로젝트를 옮길 때마다 한 번씩 돌릴 것.

**출시 전 확인 항목은 모두 끝났다.** 남은 것은 코드가 아니라 결정이다
(8장).

## 8. 결정 (2026-09-11 전부 확정)

- **`tombstone.vercel.app` 유지.** 커스텀 도메인을 붙이지 않으므로 Supabase
  Auth의 Redirect Allowlist와 구글 OAuth 리디렉션 URI는 손댈 것이 없다.
- **데스마스크 50종 유지.** 온보딩 3단계 중 하나로 남는다.

**남은 결정 없음.** #21까지 프로덕션 반영 완료.

### 출시 후에 조용히 죽는 길 하나

**GitHub는 레포가 60일간 조용하면 예약 워크플로를 자동으로 끈다.** keep-alive가
꺼지면 7일 뒤 Supabase가 정지하고, 그게 이 프로젝트를 1년간 죽여놨던 바로 그
경로다. 출시하고 손을 떼면 두 달 반쯤 뒤에 사이트가 말없이 사라진다.

막는 법은 간단하다 — 두 달에 한 번은 레포에 커밋을 남기거나, Actions 탭에서
keep-alive를 손으로 한 번 돌리면 카운터가 초기화된다. 워크플로가 실패하면
알림이 오지만, **꺼진 것에 대해서는 알림이 오지 않는다.** 그래서 적어둔다.
